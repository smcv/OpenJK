...
//	SG_ReadScreenshot(qtrue);	// qboolean qbSetAsLoadingScreen
	...
	SV_SpawnServer(sMapCmd, eForceReload_NOTHING, ToQBoolean(eSavedGameJustLoaded != eFULL) );	// note that this also trashes the whole G_Alloc pool as well (of course)		
...
static int SG_Read_Actual(unsigned int chid, void *pvAddress, int iLength, void **ppvAddressPtr, qboolean bChunkIsOptional)
{
	...
	qboolean bBlockIsCompressed = ToQBoolean((int)uiLoadedLength < 0);
