...
void G_MissileImpact( gentity_t *ent, trace_t *trace, int hitLoc=HL_NONE ) 
{
	...
	qboolean bounce = ToQBoolean( ( !other->takedamage && ( ent->s.eFlags&( EF_BOUNCE | EF_BOUNCE_HALF ) ) ) || ( ( ( trace->surfaceFlags&SURF_FORCEFIELD ) || ( other->flags&FL_SHIELDED ) ) && !ent->splashDamage&&!ent->splashRadius&&ent->s.weapon != WP_NOGHRI_STICK ) );
	...
	if ( bounce ) 
	{
		...
		G_MissileBounceEffect( ent, trace->endpos, trace->plane.normal, ToQBoolean( trace->entityNum == ENTITYNUM_WORLD ) );
